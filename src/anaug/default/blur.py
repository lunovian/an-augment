...
def blur(
    ...
    **kwargs
    ...
    - blur_radius (Union[int, float]): 
        ...
        - For 'uniform' and 'median': Kernel size. Must be positive odd integer or 1 for no blur (only for 'median').
    ...
    if blur_type == 'gaussian':
        ...
        if not isinstance(blur_radius, int) or blur_radius <= 0:
            raise ValueError("For 'median' blur_type, 'blur_radius' must be a positive odd integer or 1 for no blur.")
        if blur_radius == 1:
            return image.copy()  # No blurring needed
        if blur_radius % 2 == 0:
            raise ValueError("For 'median' blur_type, 'blur_radius' must be an odd integer.")
        if blur_radius < 3:
            raise ValueError("For 'median' blur_type, 'blur_radius' must be >= 3.")
        # Apply median blur
        if image.ndim == 2:
            blurred = cv2.medianBlur(image, blur_radius)
        elif image.ndim == 3:
            # Apply median blur to each channel independently
            blurred = np.zeros_like(image)
            for c in range(image.shape[2]):
                blurred[:, :, c] = cv2.medianBlur(image[:, :, c], blur_radius)
        else:
            raise ValueError("Input image must be either a 2D or 3D numpy array.")
        return blurred
